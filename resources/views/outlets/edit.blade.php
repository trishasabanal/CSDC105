        ...
        @can('delete', $outlet)
            <div class="card">
                ...
                <div class="card-footer">
                    <form method="POST" action="{{ route('outlets.destroy', $outlet) }}" accept-charset="UTF-8" onsubmit="return confirm(&quot;{{ __('app.delete_confirm') }}&quote;)" class="del-form float-right">
                        {{ csrf_field() }} {{ method_field('delete') }}
                        <input name="outlet_id" type="hidden" value="{{ $outlet->id }}">
                        <button type="submit" class="btn btn-danger">{{ __('app.delete_confirm_button') }}</button>
                    </form>
                    <a href="{{ router('outlets.edit', $outlet) }}" class="btn btn-link">{{ __('app.cancel') }}</a>
                </div>
            </div>
        @endcan
        @else
            ...
            <form method="POST" action="{{ route('outlets.update', $outlet) }}" accept-charset="UTF-8">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="address" class="form-control{{ $errors->has('address') ? ' is-invalid' : '' }}" name="address" rows="4">{{ old('name') }}</textarea>
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ route('outlets.show') }}" class="btn btn-link">{{ __('app.cancel') }}</a>
                    @can('delete', $outlet)
